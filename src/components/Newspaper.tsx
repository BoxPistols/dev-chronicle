...
function IconStar() {
  return (
    <svg
      className="w-3.5 h-3.5 inline-block mr-0.5 align-[-2px]"
      viewBox="0 0 24 24"
      fill="currentColor"
      stroke="none"
    >
...
function Tag({ c = "#555", children }: { c?: string; children: React.ReactNode }) {
  return (
    <span
...
function ContributionGraph({ data }: { data: ContributionCalendar }) {
...
export default function Newspaper({ gh, zenn, usernames, aiComment }: NewspaperProps) {
  ...
  const pushEvents = sortByRelevance<GitHubEvent>(
    events.filter((e) => e.type === "PushEvent")
  );
  const prEvents = sortByRelevance<GitHubEvent>(
    events.filter((e) => e.type === "PullRequestEvent")
  );
  ...
  const totalLikes = articles.reduce((s, a) => s + (a.liked_count || 0), 0);
  const hasGH = !!profile;
          ...
          {hasGH && profile && (
            <>
              ...
              <p className="text-justify text-[0.93em] leading-7 text-text dark:text-text-dark">
                GitHub上の開発者 <strong>{profile.name || ghUser}</strong>{" "}
                の直近のアクティビティを分析した。
                {totalCommits > 0 && ` 計${totalCommits}件のコミットが確認され、`}
                ...
                (() => {
                  ...
                  return (
                    <>
                      ...
                      {[...grouped.entries()]
                        .slice(0, 5)
                        .map(([repo, { fullName, commits, latestDate, pushCount }]) => (
                          <div
                            key={repo}
                          ...
                          >
                            ...
                            {commits.length > 0 ? (
                              <div className="mt-1.5">
                                ...
                                {commits.filter((c: { message: string }) => c.message)
                                  .length > 3 && (
                ...
                <>
                  ...
                  {prEvents.slice(0, 5).map((e: (typeof prEvents)[number], i: number) => {
                    ...
                    const title =
                      ...
                      (pr?.number
                        ? `#${pr.number}`
                        : e.repo?.name?.split("/")[1] || "PR");
                    const prUrl = pr?.html_url || `https://github.com/${e.repo?.name}`;
                    const repoShort = e.repo?.name?.split("/")[1];
                    return (
                      <div
                        key={i}
                        className="mb-2 pb-2 border-b border-border-light dark:border-border-dark text-[0.9em] flex items-baseline gap-1.5 flex-wrap"
                      >
                        <Tag c={prColor(e.payload?.action)}>{e.payload?.action}</Tag>
                        <a
                          href={prUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                      ...
                      {a.topics && a.topics.length > 0 && (
                        <span className="ml-2">
                          {a.topics.slice(0, 3).map((t) => (
                            <span
                              key={t}
                              className="inline-block bg-neutral-100 dark:bg-neutral-700 text-[0.9em] px-1.5 py-px rounded mr-1"
                            >
                              {t}
                            </span>
                          ))}
        ...
        <aside className="bg-surface-alt dark:bg-surface-dark-alt p-5 border border-border-light dark:border-border-dark rounded-lg text-[0.86em] h-fit">
          {hasGH && profile && (
            <div className="text-center mb-4">
              <a
                ...
                rel="noopener noreferrer"
              >
                <Avatar name={profile.name || ghUser} url={profile.avatar_url} />
              </a>
              <a
                href={`https://github.com/${ghUser}`}
                target="_blank"
          ...
          {hasGH && profile && (
            <>
              ...
              <StatRow label="アクティブリポ" value={`${activeRepos.length}件`} />
              <StatRow label="PR活動" value={`${prEvents.length}件`} />
              <StatRow label="Issue活動" value={`${issueEvents.length}件`} />
              <StatRow label="公開リポ総数" value={`${profile.public_repos || 0}件`} />
              <StatRow label="フォロワー" value={`${profile.followers || 0}人`} />
          ...
          {topLangs.length > 0 && (
            <>
              ...
              {topLangs.map(([lang, count], i) => (
                <StatRow key={lang} label={`${i + 1}. ${lang}`} value={`${count}リポ`} />
              ))}
            </>
          )}
          ...
          {hasZenn && (
            <>
              ...
              <StatRow
                label="最新投稿"
                value={articles[0] ? `${daysAgo(articles[0].published_at)}日前` : "-"}
              />
            </>
          )}
        </aside>
